...
def sysprompt(d):
    ...
    return SYS.format(ctx=ctx, rdv=rdv, obj=obj, notes=notes)
...
async def add_rdv(u: Update, c):
    ...
    save(d)
    await u.message.reply_text(f"✅ {p[2]}\n📅 {p[0]} à {p[1]}")
...
async def add_obj(u: Update, c):
    ...
    save(d)
    await u.message.reply_text(f"🎯 {desc}")
...
async def add_note(u: Update, c):
    ...
    save(d)
    await u.message.reply_text("📝 Noté !")
...
async def del_rdv(u: Update, c):
    ...
    try:
        ...
        rm = d["rdv"].pop(idx)
        save(d)
        await u.message.reply_text(f"🗑️ Supprimé: {rm['description']}")
    except: await u.message.reply_text("Numéro invalide")
...
async def handle(u: Update, c):
    if u.effective_chat.id != CID:
        await u.message.reply_text("Bot privé 🔒")
        return
    d = load()
    msg = (
        f'Message: "{u.message.text}"\n'
        f'Date: {datetime.now().strftime("%A %d/%m/%Y %H:%M")}\n'
        "Reponds naturellement. Detecte RDV/objectif/note si pertinent."
    )
    resp = await ai(msg, d)
...
async def morning(c):
    ...
    await c.bot.send_message(chat_id=CID, text=f"☀️ Salut Hraf!\n\n{r}")
    ...
    for r in d["rdv"]:
        ...
        try:
            ...
            if timedelta(0) < diff <= timedelta(minutes=30):
                await c.bot.send_message(chat_id=CID, text=f"⏰ Dans {int(diff.total_seconds()//60)} min: {r['description']}")
                r["rappel"] = True
                up = True
        except: continue
    if up: save(d)
